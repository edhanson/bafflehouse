...
def parse_to_candidates(text: str, parser_system: Optional["ParserSystem"] = None) -> List[dict]:
    ...
    text = rewrite_interaction_idioms(text)   # new verbs: light, pull, pour, wear, use, etc.
    ...
    # --------------------------------------------------------
    # Literal / typo-aware parsing first
    # --------------------------------------------------------
    ...
    if semantic_best is not None:
        ...
        # Strip just the first token-ish lead for semantic parsing.
...
def resolve_phrase_to_entities(
    ...
    """
    Resolve a noun phrase to candidate entities.

    Policy:
    - Pronouns resolve from discourse memory.
    - One-word phrases use literal token matching only.
    - Multi-word phrases require all phrase tokens to match the same alias/name.
    - Semantic similarity is only used for multi-word phrases that already pass
      literal filtering.
    """
    if not phrase:
        return []
    ...
    for eid in visible:
        ...
        if not phrase_matches_entity_literally(tokens, ent):
            continue
    ...
    if semantic_index is not None and semantic_index.embedder.enabled():
        ...
        for eid, sim in semantic_matches:
            ...
            current = scored.get(eid, 0.0)
            current += 2.0 * sim
            current += affordance_bonus(verb, ent, slot_name, prep)
            scored[eid] = current
